...
save_excel_path = f'./analysis/result/{timestamp}_BTM.xlsx'
tweets_by_topic_csv_path = "./analysis/result/2023-05-17-15-33-14_tweets_by_topic.csv"
...
def save_n_tweets_in_topic_to_csv(tweets_by_topic_df):
    ...
    n_tweets_in_topic_df = pd.DataFrame(np.zeros(total_topic_num).reshape(1, -1))
    n_tweets_in_topic_df.columns = ['topic{}'.format(i) for i in range(20)]
...
if __name__ == "__main__":
    ...
    keywords_df = save_keywords_of_every_topic_to_csv(tweets, biterm_model, tweets_by_topic_df)

    n_tweets_in_topic_df = save_n_tweets_in_topic_to_csv(tweets_by_topic_df)
